...
In order for the invariant signatures to be useful to identify similarity in images, there are two key considerations: they must vary continuously with respect to the image, either for all smooth images or for all except a set of sufficiently high codimension, and they must be robust with respect to occlusions, where one object obscures some part of another. These constraints lead to the 
This leads one naturally to consider local invariants, i.e., those defined using each point of the object as its own basepoint, together with a neighbourhood in some topology. This is sufficient to compute, e.g., derivatives of an object. 
This fails for a variety of 
However, of particular saliance for object recognition in scenes is the need to be robust to occlusions, 

The introduction of an invariant signature reduces the recognition problem to one of comparing the higher-dimensional signatures that encode the desired self-equivalences. In this paper we introduce the mathematical considerations necessary to construct invariant signatures to the planar Lie groups that can transform 2D images. We consider a wide range of planar groups, both finite-dimensional Lie groups and infinite-dimensional pseudo-Lie ones; as we shall show, the lattice structure of the set of groups influences the relationship between the sets of invariants for the different groups. We provide sample signature sets for the various groups we consider, and examples of the appearance of those signatures for transformations of a simple, smooth image, but do not consider the question of a complete practical implementation. We do not consider the matching of the signatures, for which several methods already exist, such as~\cite{}.
