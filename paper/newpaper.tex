...
\subsection{$SE(2)$}
An element of the transformation group $SE(2)$ acts on an element $(x, y)$ of $\mathbb{R}^2$ to produce $(\bar{x}, \bar{y})$ as
\begin{equation*}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation*}
Prolonging the group action up to second derivatives via the chain rule computations gives
\begin{equation}
  \begin{bmatrix}
...
We normalise in two stages. First, we set $\bar{f}_{\bar{y}} = 0$. This determines $\theta$, giving $\cos\theta = f_x/\lVert \nabla f \rVert$ and $\sin\theta = -f_y / \lVert \nabla f \rVert$. We then choose $t_x = -x\cos\theta + y\sin\theta$ and $t_y = -x\sin\theta - y \cos\theta$ to make $\bar{x} = 0$ and $\bar{y} = 0$. These three normalisations fully specify the moving frame, and so the remaining transformed derivatives are invariants. Apart from squaring $I_1$, these can be read off as:
\begin{align}
  I_0 &= f \\
  I_1 &= f_x^2 + f_y^2 \\
