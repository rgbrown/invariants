...
be written in coordinates as $(x_i, y_i)$. Then a pair of such $\mathbf{x}$ lie in $\mathbb{R}^2 \times \mathbb{R}^2$. Now consider applying the same affine transformation to each $\mathbf{x}$ independently: $\mathbf{x}_i \mapsto A \mathbf{x}_i + b$ where $A \in GL(2,\mathbb{R}$ and $b \in \mathbb{R}^2$. Then the Omega Process is a second-order differential operator defined by: 
\begin{equation}
...
The Omega Process can be applied to pairs of smooth functions as follows, writing $\frac{\partial f}{\partial x}$ as $f_x$:

\begin{equation}
\Omega_{ij} f(x_i, y_i) g(x_j, y_j) = f_{x_i}g_{y_j} - f_{y_i}g_{x_j}
\end{equation}
...
\todo[inline]{Richard: Check if the summation and negation (!) of I2 and I4 are actually invariants.} Then we can write this bit. 
I3 is Robert's one.}
...
\todo[inline]{E(2) is confusing. Robert did it by contraction, and got nice invariants. For the moving frame, the issue is that you have to consider one of the branches, since there is a reflection ambiguity. This is hacked in the PSL(3,R) case. Even then, it isn't clear that you can get the nice ones. The same issue stands for trying to extend SE(2) to E(2). 

Question: is there an extension of transvectants (change of operator) that lets you get to it that way?

However... we should do Sim(2) by adding a scaling to E(2), so we need it :(
}
...
\todo[inline]{Richard to find the moving frame version!}
