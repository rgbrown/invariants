...
Note that the denominators of all three of these equations are the same up to index. We therefore clear denominators as usual and form the three-dimensional signature ((\ref{eqn:C1})^2,(\ref{eqn:C2}),(\ref{eqn:C3})). We then project them into the unit sphere and multiply by $f$ to constrain them. This signature is continuous at all $(x,y)$ such that the three quantities are not simultaneously equal to zero, which occurs when $f_x = f_y = f_{xx} = f_{yy} = 0$, which is a codimension-1 phenomenon for images.
...
All of the second-order differential invariants of $E(2)$ play known roles in geometric analysis:
\begin{enumerate}
