...
An alternative option is to seek $\mathcal{G}$-invariant functions on $M$. Mathematical invariants provide a useful way to identify sets of objects that are the same up to some equivalence relation, since the functions are constant for objects in the same equivalence class. For a group $\mathcal{G}$, orbits of the group form the equivalence classes. For images, rather than considering the image of the object, one constructs an invariant signature of some kind, which represents the object modulo the action of elements from the relevant group. 
This has been considered extensively by Olver and co-authors for the case of outlines of objects~\cite{Calabi1998,Hoff2013}. In this paper we consider the case of signatures for images rather than outlines of curves under a variety of transformation groups.

A greyscale image can be viewed as a two-dimensional manifold embedded in a
three dimensional ambient space, with coordinates $(x, y, f(x,y))$. A
signature has coordinates $\mathcal{I}(f) = (I_1, I_2, \ldots, I_m)$ which
are functions of the derivatives of $f(x, y))$, and will therefore be a
two-dimensional manifold embedded in $\mathbb{R}^m$. Under a change of
coordinates $(x, y) \mapsto (\bar{x}, \bar{y})$ induced by a group action,
the signature of the corresponding image $\bar{f}(\bar{x}, \bar{y}) = f(x, y)$
will have the same signature.

The introduction of an invariant signature reduces the recognition problem to
one of comparing the signatures in a higher-dimensional space that encode the
desired self-equivalences. For greyscale images, it is natural to seek
signatures with three coordinates as $\mathbb{R}^3$ is the smallest space in
which the signature surfaces can reside. In this paper we introduce the
mathematical considerations necessary to construct invariant signatures to
the planar Lie groups (and some infinite-dimensional, pseudo-Lie groups too)
that can transform 2D images. As we shall show, the lattice structure of the
set of groups influences the relationship between the sets of invariants for
the different groups. We provide sample signature sets for the various groups
we consider, and examples of the appearance of those signatures for
transformations of a simple, smooth image, but do not consider the question
of a complete practical implementation. We do not consider the classification
of the signatures, for which either a standard image recognition algorithm
can be used (see, e.g.,~\cite{Zhang20} for a survey), or a matching method
such as~\cite{UsCurrents} can be used.
...
$\mathcal{I}(f)$ with the understanding that each signature component is
actually acting on the jet space $J^d(f, \Omega, k)$. The signature
$\mathcal{I}(f) \subset \mathbb{R}^m$. See Section~\ref{sec:T2} for a basic
...
and $\tilde \Omega = \varphi(\Omega)$. If the signature is not complete, then
there will be a \emph{bad set} of images that cannot be differentiated
between.
