...
Biologists differentiate between the {\em form} of an object, and the {\em shape}, being the part of form that is unaffected by size and orientation. More generally, the appearance of a fixed object in a set of images can vary markedly depending upon such variables as change of pose and relative position to the camera, even though the object itself does not change. This is even more noticeable in scenes that consist of multiple objects at different distances and orientations from the camera and any fixed background. The appearances of each individual object will then vary independently as a constant camera motion or other such transformation is applied. Typically the individual transformations will appear as the action of an element of a planar Lie group, for example the projective or affine groups, but they could also be conformal. Regardless of which group appears to act, such variations highlight the importance of `shape', as opposed to `form', in object recognition. 

For image recognition it is then natural to seek representations of objects that vary only with regard to shape change, rather than to any change in form. If the space of objects is denoted $M$ and the non-shape transformations form a group $\mathcal{G}$, then mathematically one seeks a way to study objects in $M / \mathcal{G}$. One way to achieve this is by transforming all objects to match a chosen target one by registration (the transformation of the appearance of one shape to match another, typically by gradient descent on a matching function, see e.g.~\cite{}). Hence for the group of similarity transformations (rotation and reflection, translation, and scale), a Procrustes alignment~\cite{} will transform all of the shapes into the reference frame of one template example. However, analysis on these quotient spaces is rarely simple~\cite{}, and if one is interested in the object, and not the transformation, then it necessitates a lot of computational work for relatively little benefit. An alternative option is to seek $\mathcal{G}$-invariant functions on $M$ and use the theory of invariants, which is what we consider here. In this case, rather than considering the image of the object, one works with an invariant signature of some kind, which represents the object modulo the action of elements from the relevant group. The question of how to compare invariant signatures is not considered further in this paper.

Ideally an invariant signature would be complete, so that two objects share a signature if and only if they differ only by a transformation from $\mathcal{G}$. See, e.g.,~\cite{UsMobius} and references therein for more discussion about the desirable properties of a set of invariants for image and curve analysis. However, of particular saliance for object recognition in scenes is the need to be robust to occlusions, where one object obscures some part of another. This leads one naturally to consider local invariants, i.e., those defined using each point of the object as its own basepoint, together with a neighbourhood in some topology. This is sufficient to compute, e.g., derivatives of an object. 
...
%not robust to noise. However, we'll use them anyway. 
...
The conformal diffeomorphisms provides a particularly nice example of the moving frame method. Define $\psi^{-1}(\bar{z}) = a_0 + a_1 \bar{z} + a_2 \bar{z}^2 + \dots = z$ where $z=x+iy$ and $a_i\in \mathbb{C}$. (Clearly $a_0$ is simply the translation to the origin). Note that at derivative order $n$ two real group parameters enter: $Re(a_n)$ and $Im(a_n)$. There are $n+1$ independent derivatives of $f$ of order $n$, so there must be at least $n-1$ independent invariants at each order. Working in real and imaginary coordinates, $f(x,y) = g(\bar{x}, \bar{y})$ and the moving frame computation proceeds as follows:

At first order, $(a_1 + i b_i) (\bar{x} + i \bar{y}) = x + i y$. Then:
\begin{align}
g_{\bar{x}} &= f_x x_{\bar{x}} + f_y y_{\bar{x}} \\
&= a_1 f_x + b_1 f_y \\
g_{\bar{y}} &= f_x x_{\bar{y}} + f_y y_{\bar{y}} \\
&= - b_1 f_x + 1_1 f_y 
\end{align}

Choosing $g_{\bar{x}} = 1$ and $\g_{\bar{y}} = 0$ gives $a_1 = f_x / (f_x^2 + f_y^2), \:\: b_1 = f_y / (f_x^2 + f_y^2)$. The second order computations give three equations, setting $g_{\bar{x} \bar{x}} = \g_{\bar{x} \bar{y}} = 0$ means that $g_{\bar{y} \bar{y}}$ gives the sole second-order invariant $f_{xx} + f_{yy} / (f_x^2 + f_y^2)$, which we also saw for the M\"bius group.  

At third order there are two more invariants, ***
...
The formulation we provided in the introduction considered $k$ channels of information. However, thus far we have considered only greyscale images ($k=1$). However, most images are three-channel colour (typically represented as RBG values), and multi-spectral images can have hundreds of channels of measurement. It then becomes natural to consider how to use this information in the computation of invariants. The hope is that by constructing invariants using different colour channels, fewer derivatives need to be computed. For $k$ colours, signature sets of dimension up to $k + k(k+1)/2$ can be chosen from the invariants $f^i$ and $f^i_m f^j_m$ for $i,j=1,\dots, k$. 

We briefly sketch some possibilities for some of the groups that we have considered earlier.  However, the situation is more complicated that it first appears because the information in the channels is not independent: colour is built by the intensities in the three channels, and so there is correlation between them. This means that the invariant signatures identified next are na\"ive, and should be viewed as best-case scenarios. 
