...
\begin{equation}\label{eq:prolongation}
  ...
  \end{aligned}
\end{equation}
where $\bar{x}_x = \frac{\partial \varphi_1}{\partial x}(x, y)$, et cetera. It is often convenient to work with $\varphi^{-1}$ instead, which we will do in \S\ref{sec:movingframes}.
...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
Note that once $\theta$ has been fixed, it's always possible to choose $t_x$ and $t_y$ to ensure that $\bar{x} = \bar{y} = 0$. Prolonging the group action up to second derivatives via the chain rule gives
\begin{equation}
  \begin{bmatrix}
...
\label{eqn:SE2}
 \end{equation}
where $c = \cos \theta$ and $s = \sin \theta$. We are going to choose the
moving frame to project a point in the jet space onto the submanifold
$\bar{x} = 0$, $\bar{y} = 0$, $\bar{f}_{\bar{y}} = 0$, $\bar{f}_{\bar{x}} \ge
0$. This can be thought of as rotating the image about $(\bar{x}, \bar{y})$
so that the gradient is pointing in the positive $\bar{x}$-direction, and
then translating to the origin to put it in canonical position.

This choice gives $\cos\theta = f_x / \lVert \nabla f \rVert$ and $\sin\theta = -f_y / \lVert \nabla f \rVert$, and then $t_x, t_y$ can be chosen as above. The remaining derivatives, transformed into canonical position, are therefore invariant and can be found by substituting in $c$ and $s$. Note we square $\bar{f}_{\bar{x}}$ for neatness.
\begin{align}
  I_0 &= f \\
  I_1 &= f_x^2 + f_y^2 & &(= \bar{f}_{\bar{x}}^2)\\
  I_2 &= f_x^2 f_{xx} + 2 f_x f_y f_{xy} + f_y^2 f_{yy} &&(= \bar{f}_{\bar{x}\bar{x}}) \\
  I_3 &= f_x f_y (f_{yy} - f_{xx}) + f_{xy} (f_x^2 - f_y^2) &&(= \bar{f}_{\bar{x}\bar{y}})\\
  I_4 &= f_y^2 f_{xx} - 2 f_x f_y f_{xy} + f_x^2 f_{yy} &&(= \bar{f}_{\bar{y}\bar{y}})
\end{align}

\todo[inline]{c.f. transvectant formulae}

\subsection{The Euclidean Group E(2)}
An element of the Euclidean group $E(2)$ maps an element $(x, y)$ of $\mathbb{R}^2$ to $(\bar{x}, \bar{y}$ as 
\begin{equation*}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation*}
where $\varepsilon \in \{-1, 1\}$ and the remaining parameters are the same as for $SE(2)$. If $\varepsilon = 1$ it is a rigid transformation, and if $\varepsilon = -1$ it contains a reflection.

This time, the group action is not free on $J^1$ (with elements of the form $(x, y, f, f_x, f_y)$) because there are two possible transformations that map to the normalised coordinates $(0, 0, \bar{f}, \bar{f}_{\bar{x}}, 0)$ where $\bar{f}_{\bar{x}} \ge 0$, one that reflects the gradient, and one that rotates. And first derivatives give no information about $\varepsilon$. 

So, we prolong to second derivatives, giving
\begin{equation}
  \begin{bmatrix}
  \bar f_{\bar x} \\ \bar f_{\bar y} \\ \bar f_{\bar{x}\bar{x}} \\ \bar f_{\bar{x}\bar{y}} \\ \bar f_{\bar{y}\bar{y}}
  \end{bmatrix}
...
\todo[inline]{Need to get the Js from somewhere...}
...
Recall the $SA(2)$ invariant $E$ defined in \todo[inline]{somewhere}
