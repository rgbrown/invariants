function group_experiment(f, tform, sigfun, varargin)
params = parse_inputs(varargin{:});

% Create signature objects for the function and transformed function
x = []; y = []; % to avoid stupid behaviour of syms
syms x y
[xp, yp] = tform.reverse(x, y);
fp = f(xp, yp);
sig = sigfun(f);
sigp = sigfun(fp);

% Need to create the (x,y) |-> (I0, I1, I2) mapping for the signature and
% its corresponding signature of transformed function
sig_map = @(x, y) create_sig_mapping(x, y, sig);
sigp_map = @(x, y) create_sig_mapping(x, y, sigp);

% Numerical image and transformed image functions
f_map = matlabFunction(f);
fp_map = matlabFunction(fp);

xlim = params.xlim;
ylim = params.ylim;

fscan = ~isempty(params.scanlines);
visualise();


    function visualise()
        figure(1)
        clf
        subplot(1,2,1)
        draw_image(f_map, 'xlim', xlim, 'ylim', ylim);
        subplot(1,2,2)
        draw_image(fp_map, 'xlim', xlim, 'ylim', ylim);
        figure(2)
        clf
        draw_signature(sig_map, 'xlim', xlim, 'ylim', ylim);
        axis off
        figure(3)
        clf
        draw_signature(sig_map, 'xlim', xlim, 'ylim', ylim, 'facealpha', 0.5);
        hold on
        draw_signature(sigp_map, 'xlim', xlim, 'ylim', ylim, ...
            'facecolor', 'red', 'facealpha', 0.5);  
        axis off
        if fscan
            [X, Y] = params.scanlines{:};
            figure(1)
            subplot(1,2,1)
            hold on
            plot(X, Y, 'y');
            [Xp, Yp] = tform.forward(X, Y);
            subplot(1,2,2)
            hold on
            plot(Xp, Yp, 'y')
            axis([params.xlim params.ylim]);
            [X0, X1, X2] = sig_map(X, Y);
            figure(2)
            hold on
            plot3(X0, X1, X2, 'y');
        end
    end

end

function [I0, I1, I2] = create_sig_mapping(x, y, sig)
out = sig.evaluate(x, y);
I0 = out{1};
I1 = out{2};
I2 = out{3};
end

function parameters = parse_inputs(varargin)
p = inputParser();
p.addParameter('xlim', [-1, 1]);
p.addParameter('ylim', [-1, 1]);
p.addParameter('scanlines', {});
p.addParameter('tlim', [-1, 1]);
p.addParameter('nt', 1000);
p.parse(varargin{:})
parameters = p.Results;
end























% 
% 
% 
% 
% 
% 
% f_numeric = matlabFunction(f);
% 
% 
% 
% 
% 
% 
% 
% 
% 
% 
% 
% 
% [h0, h1] = visualise_transformation(f_numeric, tform);
% figure()
% if params.sig2
%     [sig, hsig] = visualise_signatures(f, sigfun, 'tform', tform, 'facealpha', 0.5);
% else
%     [sig, hsig] = visualise_signatures(f, sigfun);
% end
% %Scan lines
% syms t
% 
% fx = 0.5*cos(3*t);
% fy = 0.5*sin(2*t);
% fsig = sig_parametric(sig, fx, fy);
% [fxp, fyp, fx0, fy0] = tform_parametric(tform, fx, fy);
% 
% t = linspace(-pi, pi, 1000);
% figure(get(hsig, 'parent'))
% hold on
% plot3(hsig, fsig{2}(t), fsig{3}(t), fsig{1}(t), 'y')
% 
% figure(get(h0, 'parent'))
% hold on
% plot(fx0(t), fy0(t), 'y')
% figure(get(h1, 'parent'))
% hold on
% plot(fxp(t), fyp(t), 'y')
% end
% 
